...
If adding tests, use **pytest**. If adding a linter, use **flake8** or **ruff**.
...
`.mcp.json` configures the **Context7** MCP server, which provides real-time SDK documentation during development. Use `mcp__context7__resolve-library-id` + `mcp__context7__query-docs` when working with Notion, Gemini, or Gradio APIs to avoid using outdated patterns.
